...
//   struct in_addr *ip_list;
   ...
   int src_ip_num;
   int action;
   char str_action[BUFSIZ];
   int check_content = 0;

	cdb_get_str(rsock, &rule_name[0], BUFSIZ, "rule-name");
	printf("rule-name: %s\n",rule_name);
	cdb_cd(rsock, "condition-clause-container/packet-security-condition/packet-security-ipv4-condition");
...
/*	cdb_cd(rsock, "../../url-category-condition");

	if (cdb_get_str(rsock, &rule_content[0], BUFSIZ, "user-defined-category") == CONFD_OK) {
		printf("Web Domain: %s\n",rule_content);
		check_content = 1;
	} else{
		printf("Web Domain: %s\n",rule_content);
...
*/
	...
	cdb_cd(rsock, "../../../time-zone/absolute-time-zone/time");
	cdb_get_datetime(rsock, &date_time, "start-time");
	printf("Start TIme: %d\n", date_time.hour);
	cdb_get_datetime(rsock, &date_time, "end-time");
	printf("End Time: %d\n", date_time.hour);

/*	cdb_cd(rsock, "../../action-clause-container/ingress-action");
	cdb_get_enum_value(rsock, &action, "ingress-action-type");
	switch(action) {
		case policy_rule_for_nsf_pass:
			str_action = "pass";
			printf("Action : pass\n");
			break;
		case policy_rule_for_nsf_drop:
			str_action = "drop";
			printf("Action : drop\n");
			break;
		case policy_rule_for_nsf_reject:
			str_action = "reject";
			printf("Action : reject\n");
			break;
		case policy_rule_for_nsf_alert:
			str_action = "alert";
			printf("Action : alert\n");
			break;
		case policy_rule_for_nsf_mirror:
			str_action = "mirror";
...
static int read_conf(struct sockaddr_in *addr)
{
    ...
    if (cdb_start_session(rsock, CDB_RUNNING) != CONFD_OK)
        return CONFD_ERR;
    cdb_set_namespace(rsock, policy_rule_for_nsf__ns);

	
	system("cp ~/suricata-3.2.1/suricata_firewall_based_time.yaml /etc/suricata/suricata_firewall_based_time.yaml");

	fp_temp = fopen("/etc/suricata/suricata.yaml.temp","w+"); 
	fp_suricata_yaml = fopen("/etc/suricata/suricata_firewall_based_time.yaml","r+");
...
//		cdb_get_str(rsock, &policy_name[0], BUFSIZ, "policy-name");
		strcpy(policy_name, "i2nsf-time-firewall");
		strncpy(temp_policy_name," - ", sizeof(temp_policy_name));
		strcat(temp_policy_name, "i2nsf-time-firewall");
		...
		rule_num = cdb_num_instances(rsock, "i2nsf-security-policy/rules");
		printf("Rule Num: %d\n", rule_num);

		for (j = 0; j < rule_num; j++) {
			cdb_pushd(rsock, "i2nsf-security-policy/rules[%d]", j);
			do_rule(rsock, fp);
			cdb_popd(rsock);
		}
//		cdb_popd(rsock);
...
//	}
	...
	system("sudo /usr/bin/suricatasc -c reload-rules /var/run/suricata/time_firewall.socket");
...
int start_confd(void)
{
    ...
    if ((status = cdb_subscribe(subsock, 3, policy_rule_for_nsf__ns, &spoint, "/i2nsf-security-policy"))
        != CONFD_OK) {
