...
/*	cdb_get_list(rsock, &dest_ip_list, &dest_ip_num, "pkt-sec-cond-ipv4-dest");
	...
	cdb_cd(rsock, "../../packet-payload-condition");

	if (cdb_get_str(rsock, &rule_content[0], BUFSIZ, "pkt-payload-content") == CONFD_OK) {
...
/*	for(i = 0; i < (dest_ip_num -1); i++) {
	...
	if (check_content == 0) {
		...
		strncat(temp_rule, "\"; content:\"abc\"; content:\"efg\"; content:\"ttt\"; nocase; sid:", sizeof(temp_rule));
	}else {
		strncat(temp_rule, " any (msg:\"", sizeof(temp_rule));
		strncat(temp_rule, rule_name, sizeof(temp_rule));
...
static int read_conf(struct sockaddr_in *addr)
{
	...
	system("cp ~/suricata-3.2.1/suricata_mail.yaml /etc/suricata/suricata_mail.yaml");

	fp_temp = fopen("/etc/suricata/suricata.yaml.temp","w+"); 
	fp_suricata_yaml = fopen("/etc/suricata/suricata_mail.yaml","r+");
...
//		cdb_get_str(rsock, &policy_name[0], BUFSIZ, "policy-name");
		strcpy(policy_name, "i2nsf-mail");
		strncpy(temp_policy_name," - ", sizeof(temp_policy_name));
		strcat(temp_policy_name, "i2nsf-mail");
...
//	}
	...
	system("sudo /usr/bin/suricatasc -c reload-rules /var/run/suricata/mail.socket");
